...
export default function EditSocialMediaPage() {
  ...
  const fetchLink = async () => {
    try {
      ...
      if (!response.ok) {
        setError(data.error || 'Không tìm thấy liên kết');
        return;
      }
  ...
  const handlePlatformChange = (platform: string) => {
    setFormData({
      ...formData,
      platform,
      icon_name: formData.icon_name || defaultIcons[platform] || '',
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || 'Cập nhật liên kết thất bại');
        return;
      }
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold glow-text mb-8">Sửa liên kết mạng xã hội</h1>
